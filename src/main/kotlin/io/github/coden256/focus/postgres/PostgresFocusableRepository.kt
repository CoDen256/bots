...
import io.github.coden256.focus.core.model.*
...
class PostgresFocusableRepository(private val db: Database): FocusableRepository {
    ...
    override fun getAttentionInstantById(focusableId: FocusableId, timestamp: Instant): Result<io.github.coden256.focus.core.model.DetailedAttentionInstant> = db.transaction {
        AttentionInstants
    ...
    override fun getLastAttentionInstant(focusableId: FocusableId): Result<io.github.coden256.focus.core.model.DetailedAttentionInstant> = db.transaction {
        AttentionInstants
            .leftJoin(Actions, {AttentionInstants.actionId}, {Actions.id})
            .selectAll()
            .where { AttentionInstants.focusableId eq focusableId.value}
    ...
    private fun mapDetailedAttentionInstant(row: ResultRow): io.github.coden256.focus.core.model.DetailedAttentionInstant? {
        val action = mapAction(row) ?: return null
        return io.github.coden256.focus.core.model.DetailedAttentionInstant(
            row[AttentionInstants.timestamp].asInstant(),
            action
        )
    }

    override fun getFocusableAttentionTimeline(focusableId: FocusableId): Result<io.github.coden256.focus.core.model.FocusableAttentionTimeline> = db.transaction {
        Focusables
            ...
            .map { (focusable: Focusable, aggregated: List<Triple<Focusable, Action?, AttentionInstant?>>) ->
                io.github.coden256.focus.core.model.FocusableAttentionTimeline(
                    focusable,
    ...
    private fun mapDetailedAttentionInstant(it: Triple<Focusable, Action?, AttentionInstant?>): io.github.coden256.focus.core.model.DetailedAttentionInstant? {
        val action = it.second
        val instant = it.third
        if (action == null || instant == null) {
            return null
        }
        return io.github.coden256.focus.core.model.DetailedAttentionInstant(instant.timestamp, action)
    }

    override fun getFocusableAttentionTimelines(): Result<List<io.github.coden256.focus.core.model.FocusableAttentionTimeline>> = db.transaction {
        Focusables
            .leftJoin(AttentionInstants, { Focusables.id }, { AttentionInstants.focusableId })
            .leftJoin(Actions, { AttentionInstants.actionId }, { Actions.id })
            .selectAll()
